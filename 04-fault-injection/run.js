...
function generateMessage() {
  ...
  var operationType;
  if (count % 5 == 0) {
    console.log("Injecting fault for message " + count);
    if (Protocol.name == "Mqtt" || Protocol.name == "MqttWs") {
      operationType = "ShutDownMqtt";
    } else {
      operationType = "ShutDownAmqp";
    }
    const closeReason = " cleanly shutdowns MQTT connection ";

    message.properties.add("AzIoTHub_FaultOperationType", operationType);
    message.properties.add("AzIoTHub_FaultOperationCloseReason", closeReason);
    message.properties.add("AzIoTHub_FaultOperationDelayInSecs", "2");
  }
